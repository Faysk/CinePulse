...
if ($LockText -notmatch [regex]::Escape("torch==$TorchVersion")) {
    throw "requirements-neural.lock does not contain torch==$TorchVersion"
}
if ($LockText -notmatch [regex]::Escape("torchaudio==$TorchaudioVersion")) {
    throw "requirements-neural.lock does not contain torchaudio==$TorchaudioVersion"
}
...
$TempRoot = Join-Path ([IO.Path]::GetTempPath()) ("cinepulse-neural-resolve-" + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $FoundUv = Get-ChildItem -LiteralPath $UvRoot -Recurse -File -Filter 'uv.exe' | Select-Object -First 1
    if (-not $FoundUv) { throw 'uv.exe not found in portable uv archive.' }
    $UvExe = $FoundUv.FullName

    Write-Host "CINEPULSE_NEURAL_RESOLVE python=$(& $VenvPython -c 'import sys; print(sys.version.split()[0])') torch=$TorchVersion index=$TorchIndex"
    ...
    if ($Joined -notmatch [regex]::Escape("torch==$TorchVersion")) {
        Write-Warning 'uv dry-run output did not echo the pinned torch version, but resolution completed successfully.'
    }
    Write-Host 'CINEPULSE_NEURAL_INSTALLER_RESOLVE_OK'
}
finally {
